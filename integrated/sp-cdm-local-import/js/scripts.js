...
jQuery( document ).ready(function($) {
			...
			$(document).on('click', '.sp-cdm-import-check-files', function () {
						...
						type: "POST",
						url: ajaxurl,
						data: {'action': 'sp_cdm_import_check_files','nonce':$('.sp-cdm-import-check-files').attr('nonce') },		
						success: function (msg) { 
			...
			jQuery.ajax({					
						type: "POST",
						url: ajaxurl,
						data: {			'action': 'sp_cdm_import_reset_session', 'nonce':$(".sp-cdm-start-import").attr('nonce')
								},
						success: function (msg) { 
