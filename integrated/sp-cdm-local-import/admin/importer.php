...
class sp_cdm_local_import_admin_integrated{
	function __construct(){
		...
		$dir = ''.$upload_dir['basedir'] .'/cdm-import/';
		cdm_mkdir($dir, 0777);
		$this->upload_dir = $dir;
	...
	function ajax_check_files(){
		...
		check_ajax_referer( 'check-local-files-nonce', 'nonce' );
		...
		echo $this->count_local_files($this->upload_dir);
		echo '</div>';
...
ini_set('upload_max_filesize', '1024M');
	  ...
	  $this->copy_local_files($this->upload_path,$_REQUEST['uid']);
...
function ajax_reset_session(){
	...
	check_ajax_referer( 'local-import-start-nonce', 'nonce' );
	  session_start();
	unset($_SESSION['continue_folder_import']);
	unset($_SESSION['folder_import_original']);
	unset($_SESSION['structure']);
...
function explodeTree($array, $delimiter = '_', $baseval = false)
{
	...
	function view(){
	 ...
	 <input input type="hidden" name="nonce" value="'.wp_create_nonce( 'local-import-nonce' ).'">
	 	 <table class="wp-list-table widefat fixed posts" cellspacing="0">
		 <tr><td style="width:200px">Local Path</td><td>'.$dir.' <a href="#" class="sp-cdm-import-check-files button" nonce="'.wp_create_nonce( 'check-local-files-nonce' ).'" >Check Files</a> <div class="sp-cdm-check-files-view"></div></td></tr>
		 ...
		 <tr><td ></td><td><input type="submit" name="import-files" class="sp-cdm-start-import" nonce="'.wp_create_nonce( 'local-import-start-nonce' ).'" value="Import"></td></tr>
		 </table>
