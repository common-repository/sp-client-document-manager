...
class cdm_recycle_bin{
		...
		function restore_recycle_bin(){
			check_ajax_referer( 'cdm_admin_view_restore_recycle_bin', 'nonce' );
		...
		function empty_recycle_bin(){
			...
			check_ajax_referer( 'cdm_admin_view_empty_recycle_bin', 'nonce' );
			global $wpdb;
	...
	<strong>' . __("Folder Parent", "sp-client-document-manager") . ': </strong> ' . stripslashes( sp_cdm_get_project_name($r[0]['parent'])) . '	
		...
		function view_recycle_bin(){
			  ...
			  <a href="#" class="button cdm-empty-recycle-bin" nonce="'.wp_create_nonce( 'cdm_admin_view_empty_recycle_bin' ).'">'.__('Empty Recycle Bin', 'sp-client-document-manager' ).'</a>    <a href="#" class="button cdm-restore-recycle-bin" nonce="'.wp_create_nonce( 'cdm_admin_view_restore_recycle_bin' ).'">'.__('Restore Everything', 'sp-client-document-manager' ).'</a>  <p><span>'.sprintf(__('Files are currently kept for %s days then removed automatically from the recycle bin. You can change the amount of days to keep files in advanced settings.', 'sp-client-document-manager' ), $retention_date).'</span></p>
