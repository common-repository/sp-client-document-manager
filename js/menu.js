jQuery( document ).ready(function($) {
		...
		return false;
			...
			function cdm_empty_recycle_bin(){
				...
				jQuery.post(ajaxurl,{
					'action': 'cdm_admin_view_empty_recycle_bin','nonce': $('.cdm-empty-recycle-bin').attr('nonce')
				}, function(response) {
						var obj = $.parseJSON(response);
						if(obj.rerun == 1){
			...
			$( document ).on( "click", ".cdm-restore-recycle-bin", function() {
					...
					if (confirm('Are you sure you want to restore the recycle bin? All files will be restored and added back to their respective places!')) {
							...
							jQuery.post(ajaxurl,{
					'action': 'cdm_admin_view_restore_recycle_bin','nonce': $('.cdm-restore-recycle-bin').attr('nonce')
				}, function(response) {
						obj = $.parseJSON(response);
						alert(obj.message );
						location.reload();
